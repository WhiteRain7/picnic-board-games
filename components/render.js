function _getColour(value,min=1,max=5){if(typeof value!=='number'||value===-1){return'#808080'}
let avg=(min+max)/2
if(value<min){return'#00BB00'}
if(value>max){return'#BB0000'}
let r,g
if(value<=avg){const ratio=(value-min)/(avg-min)
r=Math.floor(187*ratio)
g=187}else{const ratio=(value-avg)/(max-avg)
r=187
g=Math.floor(187*(1-ratio))}
const toHex=(num)=>num.toString(16).padStart(2,'0')
return `#${toHex(r)}${toHex(g)}00`}
const difficulties={[-1]:'Сложность не указана',1:'Очень простая',2:'Простая',3:'Средняя',4:'Сложная',5:'Особо сложная',}
function _gameFrom(template,data){const diffClr=_getColour(data.difficulty,1,5)
...
const estTime=(data.time.max===-1?'-':data.time.max<=60?'<1 ч':data.time.max<=120?'1-2 ч':data.time.max<=180?'2-3 ч':data.time.max<=300?'3-5 ч':'5+ ч')
const estPlrs=data.players.max===-1?'-':data.players.min===data.players.max?data.players.max.toString():`${data.players.min}-${data.players.max}`
...
el.addEventListener(eventName,event=>{if(el.hasAttribute('disabled')||el.getAttribute('aria-disabled')==='true')return
...
window.addEventListener('load',async function(){document.getElementById('req').remove()
let games=await getData()
let template=document.getElementById('game-template')
let parent=document.getElementById('games')
let list=[]
let elem
for(let game of games){elem=document.createElement('li')
