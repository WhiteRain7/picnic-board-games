...
function applyFilters(list,filters){let found=0
let nameEl
for(let entry of list){nameEl=entry.elem.querySelector('.game-name')
let visible=!0
nameEl.innerText=entry.game.name
if(filters.name.length){nameEl.innerHTML=entry.adapted.name.replaceAll(RegExp(filters.name.filter(Boolean).join('|'),'g'),word=>`<mark>${word}</mark>`)
for(let word of filters.name){if(!entry.adapted.name.includes(word)){visible=!1
break}}}
if(visible&&filters.difficulty.size){if(!filters.difficulty.has(entry.adapted.difficulty)){visible=!1}}
if(visible&&filters.time){if(entry.adapted.time>filters.time){visible=!1}}
if(visible&&filters.players){if(filters.players>=11){if(entry.game.players.max<11){visible=!1}}else if(!(entry.game.players.min<=filters.players&&filters.players<=entry.game.players.max)){visible=!1}}
if(visible&&filters.age){if(entry.game.age>filters.age){visible=!1}}
...
counter.ariaDescription=(counter.ariaDescription.split(',')[0]+', сортировка по названию - '+(dir===1?'от А до Я':'от Я до А'))
break
...
counter.ariaDescription=(counter.ariaDescription.split(',')[0]+', сортировка по времени игры - '+(dir===1?'сначала быстрые':'сначала долгие'))
break
...
counter.ariaDescription=(counter.ariaDescription.split(',')[0]+', сортировка по количеству игроков - '+(dir===1?'по возрастанию':'по убыванию'))
break}
order.forEach(item=>{container.appendChild(item.elem)})}
function initFilters(list){let adapted=list.map(entry=>{return{game:entry.game,elem:entry.elem,adapted:{name:_adapt(entry.game.name),difficulty:Math.round(entry.game.difficulty),time:entry.game.time.max===-1?Infinity:entry.game.time.max}}})
...
let filters={name:[],difficulty:new Set,time:0,players:0,age:999}
name.addEventListener('input',event=>{filters.name=_adapt(event.target.value).split(/\s+/g)
if(filters.name.length===1&&filters.name[0]===''){filters.name=[]}
name.classList.toggle('used',!!event.target.value)
applyFilters(adapted,filters)})
let i=1
let span
for(let diff of[diff1,diff2,diff3,diff4,diff5]){diff.dataset.val=i.toString()
diff.addEventListener('change',event=>{let j=parseInt(diff.dataset.val)
if(event.target.checked){filters.difficulty.add(j)}else{filters.difficulty.delete(j)}
...
applyFilters(adapted,filters)})
players.addEventListener('input',event=>{filters.players=parseInt(event.target.value)
if(filters.players===0){playersOut.innerHTML=`Любое количество`}else if(filters.players>=11){let count=adapted.reduce((s,e)=>{return s+(e.game.players.max>=11)},0)
playersOut.innerHTML=`От 11 и более <span class="soft">(${count} игр)</span>`}else{let count=adapted.reduce((s,e)=>{return s+(e.game.players.min<=filters.players&&filters.players<=e.game.players.max)},0)
...
applyFilters(adapted,filters)})
age.addEventListener('change',event=>{filters.age=parseInt(event.target.value)
...
for(let diff of[diff1,diff2,diff3,diff4,diff5]){diff.checked=!1
...
window.addEventListener('load',async function(){initFilters(await listQueue)
